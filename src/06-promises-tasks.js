...
function chainPromises(array, action) {
  function waitPromises() {
    ...
    return new Promise((resolve) => {
      array.reduce((prev, cur, i) => cur
        .then((res) => newArr.push(res))
        .then(() => { if (i === array.length - 1) resolve(newArr); })
        .catch(() => prev), 0);
    });
  }

  const result = waitPromises();
  return new Promise((resolve) => result.then((res) => {
